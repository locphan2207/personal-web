...
const animateToPage = pageName => {
  const path1 = Snap.select("#top-left-svg")
  ...
  const trans2 = "translate(-100 200)"

  switch (pageName) {
    case PAGE_NAMES.HOME_PAGE:
      path1.animate({ d: data1, transform: trans1 }, 1500, window.mina.backin)
      svg1.animate({ width: 700 }, 1500, window.mina.backin)
      break
    case PAGE_NAMES.WORK_PAGE:
      path1.animate({ d: data2, transform: trans2 }, 1500, window.mina.backin)
